...
export type PullQuote = { text: string; citation: string };
...
export type Slot =
  ...
  | { id: string; status: SlotStatus; type: "quote-block"; quote: string; citation: string; setup?: string; analysis: string; pullQuote?: PullQuote }
  ...
  | { id: string; status: SlotStatus; type: "quote-transition"; quote: string; citation: string; prose: string; setup?: string }
  ...
  for (const slot of slots) {
    ...
    switch (slot.type) {
      case "chapter-title":
        ...
        break;
      case "quote-block": {
        const pull = f["pull_quote_text"] && f["pull_quote_citation"]
          ? `, pullQuote: { text: \`${escape(f["pull_quote_text"]!)}\`, citation: \`${escape(f["pull_quote_citation"]!)}\` }`
          : "";
        items.push(`{ ${base}, type: "quote-block", quote: \`${escape(f["quote"] ?? "")}\`, citation: \`${escape(f["citation"] ?? "")}\`${f["setup"] ? `, setup: \`${escape(f["setup"]!)}\`` : ""}, analysis: \`${escape(f["analysis"] ?? "")}\`${pull} }`);
        break;
      }
      case "quote-inline":
        items.push(`{ ${base}, type: "quote-inline", quote: \`${escape(f["quote"] ?? "")}\`, citation: \`${escape(f["citation"] ?? "")}\`${f["setup"] ? `, setup: \`${escape(f["setup"]!)}\`` : ""}, analysis: \`${escape(f["analysis"] ?? "")}\` }`);
        break;
      case "bilingual-pair":
      ...
      case "quote-transition":
        items.push(`{ ${base}, type: "quote-transition", quote: \`${escape(f["quote"] ?? "")}\`, citation: \`${escape(f["citation"] ?? "")}\`, prose: \`${escape(f["prose"] ?? "")}\`${f["setup"] ? `, setup: \`${escape(f["setup"]!)}\`` : ""} }`);
        break;
      case "verb-echo": {
